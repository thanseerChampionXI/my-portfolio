...
const Blog: React.FC<BlogProps> = ({ isDarkMode = false }) => {
    return (
        <section id="blog" className={`py-8 sm:py-12 md:py-16 lg:py-20 ${isDarkMode ? 'bg-slate-800/30' : 'bg-gray-100/50'} overflow-hidden`}>
            <div className="max-w-10xl mx-auto px-4 sm:px-6 lg:px-24">
                <div className="text-center mb-16">
                    <h2 className="text-4xl font-bold mb-6 bg-gradient-to-r from-teal-400 to-emerald-400 bg-clip-text text-transparent">
                        Latest Articles
                    </h2>
                    <p className={`text-lg ${isDarkMode ? 'text-gray-300' : 'text-gray-600'} max-w-2xl mx-auto mb-8`}>
                        Insights, tutorials, and thoughts on modern web development
                    </p>
                    <Link
                        to="/blog"
                        className={`inline-flex items-center gap-2 px-6 py-3 rounded-lg ${isDarkMode ? 'bg-teal-500/20 hover:bg-teal-500/30 text-teal-300' : 'bg-blue-500/20 hover:bg-blue-500/30 text-blue-700'} transition-all duration-300 font-medium`}
                    >
                        View All Articles
                        <ArrowRight size={20} />
                    </Link>
                    ...
                    flex-shrink-0 w-80 mx-4`}
                                >
                                    <div className="space-y-4">
                                        ...
                                        <Link
                                            to={`/blog/${post.id}`}
                                            className={`flex items-center gap-2 ${isDarkMode ? 'text-teal-400 hover:text-teal-300' : 'text-purple-400 hover:text-purple-300'} transition-colors duration-300 group-hover:translate-x-1`}
                                        >
